...
typedef struct {
    ...
    uint8_t length;
    /// <summary>
    /// <para>Opcode data to be transmitted before main transfer.</para>
    /// </summary>
    const void *opcodeData;
    /// <summary>
    /// <para>Length of opcode data to be transmitted.</para>
    /// </summary>
...
/// </summary>
/// <param name="handle">SPI handle to perform the transfer on.</param>
/// <param name="opcode">A pointer to the opcode to be written.</param>
/// <param name="opcodeLength">The length of the opcode to be written.</param>
...
static inline int32_t SPIMaster_WriteAsync(
    SPIMaster  *handle,
    const void *opcode, uint8_t opcodeLength,
...
static inline int32_t SPIMaster_WriteThenReadAsync (
    SPIMaster  *handle,
    const void *opcode   , uint8_t opcodeLength,
    const void *writeData, uint8_t writeLength,
    void       *readData , uint8_t readLength,
    void      (*callback)(int32_t status, uintptr_t dataCount));
    ...
    void      (*callback)(int32_t status, uintptr_t dataCount))
{
    SPITransfer transfer[2] = {
        { .writeData = writeData, .readData = NULL    , .length = writeLength, .opcodeData = opcode, .opcodeLength = opcodeLength },
        { .writeData = NULL     , .readData = readData, .length = readLength , .opcodeData = NULL  , .opcodeLength = 0            },
    };
